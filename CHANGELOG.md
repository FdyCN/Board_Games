...
- **情书（Love Letter）**：引擎已实现（隐藏信息 + 回合制 + 淘汰制）。3 人 PPO vs 2 随机对手胜率 **~76%**（v1 绝对视角 77%，v2 相对视角 75.5%，均 200 局）。v2 相对视角下座位胜率标准差仅 **1.6%**（座位无关性好）；追加 `eliminate_opponent` 奖励与 800 迭代未进一步提升胜率。
