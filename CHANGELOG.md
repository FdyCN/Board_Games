...
- **情书（Love Letter）**：引擎已实现（隐藏信息 + 回合制 + 淘汰制）。3 人 vs 2 随机对手胜率：**PPO 系列稳定 75~77%**（v1 绝对视角 77%、v2 相对视角 75.5%、对手池+信念+上帝视角 77%）；**NFSP 约 59%**（低于 PPO）；**手写规则 bot ≈ 66~74%（平均约 71%）**。校准结论：PPO 77% 高于「明显正确规则」的启发式基线约 6 个点，说明 PPO 学到了真实的东西、且 vs 随机的剩余空间有限。上帝视角辅助头能学会推断对手手牌（aux_loss 2.03→1.63）但未转化为胜率。
