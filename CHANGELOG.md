...
- **情书（Love Letter）**：引擎已实现（隐藏信息 + 回合制 + 淘汰制）。3 人 vs 2 随机胜率：**对手池（联赛）GRU 模型 82~87%（最佳）**、手写规则 bot ~71%、随机 33%。关键结论：**纯 PPO 对称自对弈学不动（价值函数 EV≈0，模型停在随机 25-33%），必须建立对手池（把 71% 的手写规则 bot 放进池子当对手），模型才能学会并反超**。另：私密知识追踪（known 矩阵）+ GRU 序列编码 + 有序事件时间线均已实现。
