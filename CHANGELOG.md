...
- **情书（Love Letter）**：引擎已实现（隐藏信息 + 回合制 + 淘汰制）。3 人 vs 2 随机胜率：**对手池（联赛）GRU 模型 82~87%**、**强贝叶斯规则 bot 84.5%**、弱规则 bot ~71%、随机 33%。关键结论：纯 PPO 对称自对弈学不动（价值函数 EV≈0，模型停在随机），**必须建立对手池（把规则 bot 放进池子）才能学会**。联赛模型与强贝叶斯 bot 直接对打 **~48% vs ~49%（基本打平）**，证明它不是「矮子里拔高子」，而是真正达到了强规则 bot 的水平。私密知识追踪（known 矩阵）+ GRU 序列编码 + 有序事件时间线均已实现。
